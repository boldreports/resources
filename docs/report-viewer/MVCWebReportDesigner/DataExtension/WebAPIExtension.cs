...
namespace BoldReports.DataExtensions.WebAPI
{
    public class WebAPIExtension : BoldReports.Data.IDataExtension
    {
        ...
        public object GetData(out string error)
        {
            try
            {
                ...
                return this.GetReportData(
                    Newtonsoft.Json.JsonConvert.DeserializeObject<dynamic>(this.InvokeServiceRequest(this.ConnectionProperties.ConnectionString).ToString()));
            }
            catch (Exception ex)
            {
                error = ex.Message;
        ...
        public SchemaData GetDataSourceSchema(out string error)
        {
            try
            {
                error = string.Empty;
        ...
        private string UpdateQueryParameter(string queryInfo)
        {
            try
            {
                Regex queryParam = new Regex(@"(\@)([a-zA-Z0-9]+)", RegexOptions.IgnoreCase);
                var cmdObj = Newtonsoft.Json.JsonConvert.DeserializeObject<QueryDataInfo>(queryInfo);

                if (cmdObj != null && this.Command.Parameters != null && this.Command.Parameters.Count > 0)
                {
                    if (cmdObj.Parameters != null && cmdObj.Parameters.Any())
                    {
                        foreach (Match match in queryParam.Matches(queryInfo))
                        {
                            if (match.Success)
                            {
                                foreach (var cmdParam in this.Command.Parameters)
                                {
                                    ...
                                    {
                                        queryInfo = queryInfo.Replace(cmdParam.Name, cmdParam.Value == null ? string.Empty : cmdParam.Value.ToString());
                                    }
                                }
                            }
                        }
                    ...
                    else
                    {
                        ...
                        queryInfo = Newtonsoft.Json.JsonConvert.SerializeObject(cmdObj);
                    }
                }
            }
            catch
        ...
        Dictionary<string, object> GetRecords(object data)
        {
            ...
            {
                var property = JsonConvert.DeserializeObject<Dictionary<string, object>>(data.ToString());
                Dictionary<string, object> fieldValues = new Dictionary<string, object>();
